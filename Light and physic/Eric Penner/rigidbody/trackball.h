...
//Math functions
    ...
    inline
    static void Matrix3fMulVector3f(Matrix3fT* mat, Vector3fT* NewObj)
    {
		Vector3fT temp; 

		temp.s.X = mat->s.M00 * NewObj->s.X + mat->s.M01 * NewObj->s.Y + mat->s.M02 * NewObj->s.Z;
        temp.s.Y = mat->s.M10 * NewObj->s.X + mat->s.M11 * NewObj->s.Y + mat->s.M12 * NewObj->s.Z;
        temp.s.Z = mat->s.M20 * NewObj->s.X + mat->s.M21 * NewObj->s.Y + mat->s.M22 * NewObj->s.Z;
...
// 8<--Snip here if you have your own math types/funcs-->8 
    ...
    typedef class Trackball
    {
        ...
        public:
            //Create/Destroy
                    Trackball(GLfloat NewWidth, GLfloat NewHeight);
                   ~Trackball() { /* nothing to do */ };
