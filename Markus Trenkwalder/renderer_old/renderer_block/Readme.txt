...
class BaseBlockShader {
public:
	static const int  varying_count = 0;
	static const bool interpolate_z = false;
	static const bool high_precision_z = false;
	static const bool high_precision_varyings = false;

	// do an early depth test. return true if it passed or false if the whole
	// block can be discarded.
	static bool early_depth_test(int x, int y, int z) { return true; }

	template <bool allin>
	static void block(
		const Rasterizer::PixelBlock block,
		const Rasterizer::SkipFlag skip_flag,
		int bx, int by)
	{
		// loop over all pixels in the block testing skip_flag if the pixel
		// should actually be drawn. If the template parameter allin is true
		// no pixels will be skipped.
		
		// Neither the renderer nor the rasterizer know about the render
		// targets. It is the duty of the block shader to compute the location
		// where the individual pixels need to be stored.
	}
