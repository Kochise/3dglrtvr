...
inline BOOL AtlGetFilePidl(LPCTSTR pstrFileName, LPITEMIDLIST* pidl)
{
   ...
   *pidl = NULL;
   ...
   if( FAILED(spFolder->ParseDisplayName(NULL, NULL, pwchPath, NULL, pidl, NULL)) ) return FALSE;   
   return TRUE;
};
...
public:
   ...
   BOOL Populate(int csidl = CSIDL_DESKTOP)
   {
      ...
      return Populate(spDesktop, pidl, csidl == CSIDL_DESKTOP ? NULL : static_cast<LPCITEMIDLIST>(pidl));
   }
   ...
   BOOL Populate(LPCTSTR pstrPath)
   {
      ...
      if( FAILED( spDesktop->ParseDisplayName(NULL, NULL, const_cast<LPOLESTR>(pwstrPath), &uEaten, &pidl, NULL) ) ) return FALSE;
