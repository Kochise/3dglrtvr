...
#define SHCNF_ACCEPT_NON_INTERRUPTS 0x0002 
      ...
      LPARAM lOffsetCustom)
   {
      ...
      for( int j = 0; j < nCount; j++ ) {
         ...
         case TBI_STD:
            (ptbb + j)->iBitmap = (int) lOffsetFile + pButtons[j].tb.iBitmap;
            break;
         case TBI_VIEW:
            (ptbb + j)->iBitmap = (int) lOffsetView + pButtons[j].tb.iBitmap;
            break;
         case TBI_LOCAL:
            (ptbb + j)->iBitmap = (int) lOffsetCustom + pButtons[j].tb.iBitmap;
            break;
         }
...
public:
   ...
   STDMETHOD(ItemCount)(UINT uFlags, int* pcItems)
   {
      ...
      if( (uFlags & SVGIO_FLAG_VIEWORDER) != 0 ) uState |= LVNI_VISIBLEORDER;
...
{
public:
   STDMETHOD(GetPaneState)(REFEXPLORERPANE ep, EXPLORERPANESTATE *peps) PURE;
};

#endif  // IExplorerPaneVisibility
...
{
public:
   STDMETHOD(Next)(ULONG celt, IExplorerCommand** pUICommand, ULONG* pceltFetched) PURE;
   STDMETHOD(Skip)(ULONG celt) PURE;
   STDMETHOD(Reset)(void) PURE;
   STDMETHOD(Clone)(IEnumExplorerCommand **ppenum) PURE;
};
...
{
public:
   STDMETHOD(GetTitle)(IShellItemArray* psiItemArray, LPWSTR* ppszName) PURE; 
   STDMETHOD(GetIcon)(IShellItemArray *psiItemArray, LPWSTR* ppszIcon) PURE; 
   STDMETHOD(GetToolTip)(IShellItemArray* psiItemArray, LPWSTR* ppszInfotip) PURE; 
   STDMETHOD(GetCanonicalName)(GUID *pguidCommandName) PURE;
   STDMETHOD(GetState)(IShellItemArray* psiItemArray, BOOL fOkToBeSlow, EXPCMDSTATE* pCmdState) PURE;
   STDMETHOD(Invoke)(IShellItemArray* psiItemArray, IBindCtx *pbc) PURE; 
   STDMETHOD(GetFlags)(EXPCMDFLAGS* pFlags) PURE;
   STDMETHOD(EnumSubCommands)(IEnumExplorerCommand** ppEnum) PURE; 
};
...
{
public:
   STDMETHOD(GetCommands)(IUnknown* punkSite, REFIID riid, LPVOID* ppv) PURE;  
   STDMETHOD(GetCommand)(REFGUID rguidCommandId, REFIID riid, LPVOID* ppv) PURE;  
};

#endif  // IExplorerCommandProvider
...
{
public:
   STDMETHOD(Rearrange)(LPARAM lParamSort) PURE;
   STDMETHOD(GetArrangeParam)(LPARAM* plParamSort) PURE;
   STDMETHOD(ArrangeGrid)(void) PURE;
   STDMETHOD(AutoArrange)(void) PURE;
   STDMETHOD(GetAutoArrange)(void) PURE;
   STDMETHOD(AddObject)(LPITEMIDLIST pidl, UINT* puItem) PURE;
   STDMETHOD(GetObject)(LPITEMIDLIST pidl, UINT puItem) PURE;
   STDMETHOD(RemoveObject)(LPITEMIDLIST pidl, UINT* puItem) PURE;
   STDMETHOD(GetObjectCount)(UINT* puCount) PURE;
   STDMETHOD(SetObjectCount)(UINT uCount, UINT dwFlags) PURE;
   STDMETHOD(UpdateObject)(LPITEMIDLIST pidlOls, LPITEMIDLIST pidlNew, UINT* puItem) PURE;
   STDMETHOD(RefreshObject)(LPITEMIDLIST pidl, UINT* puItem) PURE;
   STDMETHOD(SetRedraw)(BOOL bRedraw) PURE;
   STDMETHOD(GetSelectedCount)(UINT* puSelected) PURE;
   STDMETHOD(GetSelectedObjects)(LPITEMIDLIST** ppidl, UINT* puItems) PURE;
   STDMETHOD(IsDropOnSource)(IDropTarget* DropTarget) PURE;
   STDMETHOD(GetDragPoint)(POINT* ppt) PURE;
   STDMETHOD(GetDropPoint)(POINT* ppt) PURE;
   STDMETHOD(MoveIcons)(IDataObject* DataObject) PURE;
   STDMETHOD(SetItemPos)(LPITEMIDLIST pidl, POINT* ppt) PURE;
   STDMETHOD(IsBkDropTarget)(IDropTarget* DropTarget) PURE;
   STDMETHOD(SetClipboard)(BOOL bMove) PURE;
   STDMETHOD(SetPoints)(IDataObject* DataObject) PURE;
   STDMETHOD(GetItemSpacing)(ITEMSPACING* spacing) PURE;
   STDMETHOD(SetCallback)(IShellFolderViewCB* pNewCB, IShellFolderViewCB** ppOldCB) PURE;
   STDMETHOD(Select)(UINT dwFlags) PURE;
   STDMETHOD(QuerySupport)(UINT* pdwSupport) PURE;
   STDMETHOD(SetAutomationObject)(IDispatch* disp) PURE;
};
