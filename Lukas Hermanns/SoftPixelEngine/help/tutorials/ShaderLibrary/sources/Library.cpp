...
{
    const f32 Morphing = SIN((f32)spDevice->getMilliseconds() / 25);
...
void FECallbackOGL(video::ShaderTable* ShdTable, const scene::MaterialNode* Object)
{
    ...
    ShdTable->getVertexShader()->setConstant(
        "LayerInterpolation", (f32)FurLayerIndex / FurLayerCount
    );
...
void FurEffect::Update()
{
    ...
    const f32 Angle = (f32)spDevice->getMilliseconds() / 15;
    
    // This factor occurs that the hair is blowing in the wind.
    ShdVertex_->setConstant(
        "TexCoordTranslation", dim::vector3df(SIN(Angle), COS(Angle), 0) * 0.01
    );
...
void FurEffect::MakeHairs(video::Texture* Tex)
{
    ...
    for (s32 y = 0, x; y < Tex->getSize().Height; ++y)
    {
        for (x = 0; x < Tex->getSize().Width; ++x)
        {
            ...
            {
                Color = Tex->getPixelColor(dim::point2di(x, y));
                Color.Alpha = 0;
                Tex->setPixelColor(dim::point2di(x, y), Color);
            }
        }
    }
...
bool DepthOfField::Load()
{
    ...
    Object_ = spScene->createMesh(scene::MESH_CUBE);
    Object_->setPosition(dim::vector3df(-0.7, 0, 2));
...
void BumpMapping::Update()
{
    ...
    const f32 Angle = (f32)spDevice->getMilliseconds() / 25;
...
{
    WaterPlane_->getMeshBuffer(0)->textureTranslate(0, dim::point2df(0, 0.005));
    WaterPlane_->getMeshBuffer(0)->textureTranslate(1, dim::point2df(0, 0.005));
