...
enum ESceneGraphs
{
    SCENEGRAPH_USERDEFINED,           //!< User defined scene manager.
    SCENEGRAPH_SIMPLE,                //!< Simple default scene manager.
    SCENEGRAPH_SIMPLE_CHILDTREE,      //!< Simple scene manager with child tree hierarchy.
    //SCENEGRAPH_EXPANSIVE,             //!< Expansive scene manager for extremly large scenes.
    //SCENEGRAPH_EXPANSIVE_CHILDTREE,   //!< Expansive scene manager for extremly large scenes with child tree hierarchy.
};
...
class SP_EXPORT SceneGraph : public RenderNode
{
    ...
    public:
        ...
        */
        virtual Mesh* loadScene(
            ...
            const ESceneFileFormats Format = SCENEFORMAT_UNKNOWN, const s32 Flags = SCENEFLAG_ALL
        );
        ...
        */
        ...
        /**
        Updates each animation. To optimize the performance use the "updateAnimation" function of each animatable object
        which is needed to be animated. Otherwise the engine goes through all animatable objects all the time.
        */
    ...
    protected:
        ...
        template <class T, class L> bool removeObjectFromList(SceneNode* Object, std::list<L*> &SearchList, bool isDelete = false)
        {
            if (!Object)
                return false;
            
            for (typename std::list< L*, std::allocator<L*> >::iterator it = SearchList.begin(); it != SearchList.end(); ++it)
            {
                if (*it == Object)
                {
                    SearchList.erase(it);
