...
/* Line-line distance & intersection */
    ...
    const dim::line3df &LineA, const dim::line3df &LineB, dim::vector3df &PointP, dim::vector3df &PointQ)
{
    ...
    if (a <= FLOAT_ROUND && e <= FLOAT_ROUND)
    {
    ...
    const dim::line3df &LineA, const dim::line3df &LineB, dim::vector3df &Intersection)
{
    ...
    if (getLineLineDistanceSq(LineA, LineB, PointP, PointQ) <= FLOAT_ROUND && getDistanceSq(PointP, PointQ) <= FLOAT_ROUND)
    {
...
/* Line-box intersection (when box is transformed, transform the "Line" with the inverse box matrix) */
    ...
    const dim::obbox3df &BoxA, const dim::obbox3df &BoxB)
{
    ...
    for (i = 0; i < 3; ++i)
        for (j = 0; j < 3; ++j)
            AbsR[i][j] = math::Abs(R[i][j]) + math::FLOAT_ROUND;
