...
void ImageLoaderWAD::readTextures()
{
    ...
    for (s32 i = 0; i < Header_.CountTextures; ++i)
    {
        ...
        FinalTexture = __spVideoDriver->createTexture(
            dim::size2di(Texture.Width, Texture.Height), video::PIXELFORMAT_RGB, ImageBuffer, Name
        );
            ...
            case '{':
            {
                ...
                if (cr != video::color(0, 0, 255))
                {
                    ...
                    if (cl == video::color(255))
                        FinalTexture->setColorKeyAlpha(video::BLENDING_DARKNESS);
                    else
                        FinalTexture->setColorKeyAlpha(video::BLENDING_BRIGHTNESS);
                }
                else
                {
                    /* Make color-key */
                    ...
                    for (y = 0; y < Texture.Height; ++y)
                    {
                        ...
                        {
                            if (FinalTexture->getPixelColor(dim::point2di(x, y)) == video::color(0, 0, 255, 0))
                                FinalTexture->setPixelColor(dim::point2di(x, y), video::color(0, 0, 0, 0));
                        }
                    }
