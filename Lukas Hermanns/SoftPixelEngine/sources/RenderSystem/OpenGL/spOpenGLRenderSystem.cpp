...
{
    f32 Depth;
    glReadPixels(Position.X, __Screen_Height - Position.Y, 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &Depth);
    return Depth;
}
    ...
    Font* FontObject, const dim::point2di &Position, const io::stringc &Text, const color &Color)
{
    ...
    if (!FontObject->getTexture())
    {
        ...
        glBitmap(
            0, 0, 0, 0,
            Position.X - __Screen_Width/2,
            isInvertScreen_ ? (Position.Y - __Screen_Height/2) : (-Position.Y + __Screen_Height/2 - FontSize.Height),
            0
        );
