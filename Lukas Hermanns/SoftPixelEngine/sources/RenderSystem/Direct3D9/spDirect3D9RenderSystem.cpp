...
void Direct3D9RenderSystem::setFog(const EFogTypes Type)
{
    ...
    switch (FogType_ = Type)
    {
        ...
        case FOG_STATIC:
        {
            ...
            /* Range settings */
            D3DDevice_->SetRenderState(D3DRS_FOGDENSITY, *(DWORD*)&FogRange_);
            D3DDevice_->SetRenderState(D3DRS_FOGSTART, *(DWORD*)&FogNear_);
            D3DDevice_->SetRenderState(D3DRS_FOGEND, *(DWORD*)&FogFar_);
        }
        break;
...
void Direct3D9RenderSystem::drawStencilShadow(const video::color &Color)
{
    ...
    /* Draw the rectangle */
    beginDrawing2D();
    draw2DRectangle(dim::rect2di(0, 0, __Screen_Width, __Screen_Height), Color);
    endDrawing2D();
...
void Direct3D9RenderSystem::pushViewport()
{
    D3DVIEWPORT9 d3dViewport;
    D3DDevice_->GetViewport(&d3dViewport);
    
    StackViewport_[0] = d3dViewport.X;
    StackViewport_[1] = d3dViewport.Y;
    StackViewport_[2] = d3dViewport.Width;
    StackViewport_[3] = d3dViewport.Height;
}

void Direct3D9RenderSystem::popViewport()
{
    setViewport(
        dim::point2di(StackViewport_[0], StackViewport_[1]), dim::size2di(StackViewport_[2], StackViewport_[3])
    );
}
...
void Direct3D9RenderSystem::beginDrawing2D()
{
    ...
    #ifdef __DRAW2DXYZ__
    Matrix2D_.make2Dimensional(__Screen_Width, -__Screen_Height);
    ...
    Font* FontObject, const dim::point2di &Position, const io::stringc &Text, const color &Color)
{
    ...
    if (Position.X > __Screen_Width || Position.Y > __Screen_Height || Position.Y < -FontSize.Height)
        return;
    
    /* Temporary variabels */
    RECT rc;
    rc.left = Position.X, rc.top = Position.Y;
    rc.right = __Screen_Width, rc.bottom = __Screen_Height;
...
void Direct3D9RenderSystem::setupTextureAttributes(
