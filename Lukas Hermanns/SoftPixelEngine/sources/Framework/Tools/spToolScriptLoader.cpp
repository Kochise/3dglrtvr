...
{
    scene::AnimationNode* NodeAnim = (scene::AnimationNode*)Obj->addAnimation(scene::ANIMATION_NODE);
    
    for (std::list<SAnimationFrame>::iterator it = CurAnim_.Frames.begin(); it != CurAnim_.Frames.end(); ++it)
    {
        NodeAnim->addSequence(
            it->Pos, it->Rot, it->Scl, it->Speed
        );
    }
    
    NodeAnim->setSpeed(CurAnim_.Speed);
    
    if (CurAnim_.Play == "oneshot")             NodeAnim->play(scene::ANIM_ONESHOT);
    else if (CurAnim_.Play == "oneloop")        NodeAnim->play(scene::ANIM_ONELOOP);
    else if (CurAnim_.Play == "loop")           NodeAnim->play(scene::ANIM_LOOP);
    else if (CurAnim_.Play == "pingpong")       NodeAnim->play(scene::ANIM_PINGPONG);
    else if (CurAnim_.Play == "pingpongloop")   NodeAnim->play(scene::ANIM_PINGPONG_LOOP);
