...
#ifdef _DEB_PF_
s32 PathNode::StcNr_ = 0;
#endif

PathNode::PathNode()
    : BasicObject(), WayCosts_(0), DirectDistance_(0), Predecessor_(0)
{
}
PathNode::PathNode(const dim::vector3df &Position, void* Data)
    : BasicObject(), Position_(Position), WayCosts_(0), DirectDistance_(0), Predecessor_(0)
{
    setUserData(Data);
    #ifdef _DEB_PF_
    _TEST_ = false;
...
PathEdge::PathEdge()
    : BasicObject(), From_(0), To_(0), Distance_(0), Adjusted_(false)
{
}
PathEdge::PathEdge(PathNode* From, PathNode* To, bool Adjusted)
    : BasicObject(), From_(From), To_(To), Adjusted_(Adjusted)
{
    if (From_ && To_)
    {
...
PathNode* PathGraph::getNextNode()
{
    ...
    #ifdef _DEB_PF_
    NextNode->_TEST_ = true;
    NextNode->Nr_ = ++PathNode::StcNr_;
    #endif
