...
vector<GceControlPoint*> gCp;
vector<GmInteractiveWidget*> gWidgets;

// frame timing stuff
static GLint T0 = 0;
int framesDrawn;
GceControlPoint::CurveType gCurveMode = GceControlPoint::CM_HERMITE_CURVE;
...
/* The curve editor GUI manager */
GceCurveUI cui;

void gce_print_usage()
{
   cout << "curveEditor v0.1" << endl;
   cout << "usage: curveEditor" << endl;
   cout << endl;
}

void gceRenderGrid(GmTypes::RenderOptionType renderOptions)
{
...
void gceRenderWidgets(GmTypes::RenderOptionType renderOptions)
{
   for(vector<GmInteractiveWidget*>::iterator w = gWidgets.begin();
       w != gWidgets.end(); w++)
   {
...
void gceRemoveWidgets(GceControlPoint::CurveType ct)
{
   switch(ct)
   {
      case GceControlPoint::CM_HERMITE_CURVE:
         cui.removeHermiteCurveWidgets();
         break;
      case GceControlPoint::CM_BEZIER_CURVE:
         cui.removeBezierCurveWidgets();
         break;
      case GceControlPoint::CM_HERMITE_SPLINE:
         cui.removeHermiteSplineWidgets();
         break;
      case GceControlPoint::CM_BEZIER_SPLINE:
         cui.removeBezierSplineWidgets();
         break;
      default:
         break;
   };
...
void gcePickingProcess(GLint hits)
{
   GLuint* ptr;
...
//    cout << "Hits: " << hits << endl;
   ...
   for(int i = 0; i < hits; i++)
   {
      //ptr += 3;
      numberOfNames = *ptr;
      //cout << "numberOfNames: " << numberOfNames << endl;
...
static void glDisplay()
{
   ...
   gceRenderGrid(renderOptions);
   gceRenderWidgets(renderOptions);

   switch(gCurveMode)
   {
      case GceControlPoint::CM_HERMITE_CURVE:
         cui.renderHermiteCurve(renderOptions);
         break;
      case GceControlPoint::CM_BEZIER_CURVE:
         cui.renderBezierCurve(renderOptions);
         break;
      case GceControlPoint::CM_HERMITE_SPLINE:
         cui.renderHermiteSpline(renderOptions);
         break;
      case GceControlPoint::CM_BEZIER_SPLINE:
         cui.renderBezierSpline(renderOptions);
         break;
      default:
         cout << "Unknown curve mode, render failed." << endl;
   }
...
static void glReshape(int width, int height)
{
   ...
   glMatrixMode(GL_PROJECTION);
   glLoadIdentity();
   glOrtho(-gScaleFactor, gScaleFactor,
           -gScaleFactor, gScaleFactor,
           -1.0, 1.0);
   glMatrixMode(GL_MODELVIEW);
   glLoadIdentity();
...
static void glKey(unsigned char key, int x, int y)
{
   switch(key)
   {
      ...
      case 'c':
         ...
         break;
      case '1':
         gceRemoveWidgets(gCurveMode);
         gCurveMode = GceControlPoint::CM_HERMITE_CURVE;
         cui.initHermiteCurveWidgets();
         break;
      case '2':
         gceRemoveWidgets(gCurveMode);
         gCurveMode = GceControlPoint::CM_BEZIER_CURVE;
         cui.initBezierCurveWidgets();
         break;
      case '3':
         gceRemoveWidgets(gCurveMode);
         gCurveMode = GceControlPoint::CM_HERMITE_SPLINE;
         cui.initHermiteSplineWidgets();
         break;
      case '4':
         gceRemoveWidgets(gCurveMode);
         gCurveMode = GceControlPoint::CM_BEZIER_SPLINE;
         cui.initBezierSplineWidgets();
         break;
      default:
