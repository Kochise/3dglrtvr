...
int init_texture(){
	...
	glBmpInit(&panneau);
	hResource = LoadResource(NULL,FindResource(NULL,"#1003",RT_RCDATA));
	imgPtr = (char*)LockResource(hResource);
	if(!glBmpMemLoadTGA(&panneau,imgPtr)){MessageBox(NULL,"1","",MB_OK); return 0;}
	...
	glBmpInit(&metal);
	hResource = LoadResource(NULL,FindResource(NULL,"#1002",RT_RCDATA));
	imgPtr = (char*)LockResource(hResource);
	if(!glBmpMemLoadTGA(&metal,imgPtr)){MessageBox(NULL,"2","",MB_OK); return 0;}
	FreeResource(hResource);
	...
	glBmpInit(&arbre);
	hResource = LoadResource(NULL,FindResource(NULL,"#1004",RT_RCDATA));
	imgPtr = (char*)LockResource(hResource);
	if(!glBmpMemLoadTGA(&arbre,imgPtr)){MessageBox(NULL,"3","",MB_OK); return 0;}
	FreeResource(hResource);
