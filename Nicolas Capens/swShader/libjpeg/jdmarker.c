...
#define INPUT_VARS(cinfo)  	struct jpeg_source_mgr * datasrc = (cinfo)->src;  	const JOCTET * next_input_byte = datasrc->next_input_byte;  	size_t bytes_in_buffer = datasrc->bytes_in_buffer
 ...
 */
#define MAKE_BYTE_AVAIL(cinfo,action)  	if (bytes_in_buffer == 0) {  	  if (! (*datasrc->fill_input_buffer) (cinfo))  	    { action; }  	  INPUT_RELOAD(cinfo);  	}  	bytes_in_buffer--
 ...
 */
#define INPUT_2BYTES(cinfo,V,action)  	MAKESTMT( MAKE_BYTE_AVAIL(cinfo,action); 		  V = ((unsigned int) GETJOCTET(*next_input_byte++)) << 8; 		  MAKE_BYTE_AVAIL(cinfo,action); 		  V += GETJOCTET(*next_input_byte++); )
