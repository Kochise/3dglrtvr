...
MeshLoaderMS3D::MeshLoaderMS3D() : MeshLoader()
{
    init();
}
...
bool MeshLoaderMS3D::loadModelData(io::stringc Filename)
{
    ...
    {
        io::Log::error("MS3D model has wrong identity, must be 'MS3D000000'");
        return false;
    }
    ...
    {
        io::Log::error(
            "MS3D model has unsupported version (" + io::stringc(pHeader->Version) +
...
void MeshLoaderMS3D::buildAnimation()
{
    ...
    AnimationSkeletal* AnimSkeletal = 0;
    ...
    AnimSkeletal = (AnimationSkeletal*)Anim_;
    ...
    for (i = 0; i < CountOfJoints_; ++i)
    {
        ...
        AnimBone = AnimSkeletal->addBone(
            AnimSkeletal->getBone(CurJoint->Parent + 1),    // Parent bone
            CurJoint->Translation,                          // Translation (location)
            dim::quaternion(LocalMatrix),                   // Quaternion
            1.0f,                                           // Scale
            CurJoint->Name                                  // Bone name
        );
        
        LocalMatrix.setPosition(CurJoint->Translation);
        
        // Get the vertices list
        fillBoneWeights(i, VertexWeights);
        
        // Add the vertices indices
        AnimBone->setupVertexWeights(VertexWeights);
        
        // Clear the indices lists
        VertexWeights.clear();
        ...
        for (j = 0; j < CurJoint->RotationKeyframes.size(); ++j)
        {
            ...
            if (j < CurJoint->RotationKeyframes.size() - 1)
                Time = CurJoint->RotationKeyframes[j + 1].Time - CurJoint->RotationKeyframes[j].Time;
            else
                Time = CurJoint->RotationKeyframes[0].Time - CurJoint->RotationKeyframes[j].Time;
            
            // Add the bone transformation keyframe
            AnimBone->addKeyframe(Pos, Rot, 1.0f, Time * 1000.0f);
        }
...
void MeshLoaderMS3D::fillBoneWeights(
    const s32 JointIndex, std::vector<SVertexWeight> &VertexWeights)
{
    // Temporary variables
    s32 i, j, k;
    s32 MdlVertex;
        ...
        for (j = 0; j < pMeshes_[i].CountOfTriangles; ++j)
        {
            ...
            for (k = 0; k < 3; ++k, ++MdlVertex)
            {
                ...
                if (pVertices_[VertexIndex].BoneID == JointIndex)
                    VertexWeights.push_back(SVertexWeight(Mesh_->getMeshBuffer(i), MdlVertex));
            } // /vertices (3)
        } // /triangles
    } // /surfaces
}
