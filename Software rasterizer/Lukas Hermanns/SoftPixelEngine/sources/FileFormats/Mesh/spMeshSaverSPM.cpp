...
void MeshSaverSPM::writeChunkSubMesh(Mesh* SubMesh)
{
    ...
    // Set the flags
    s16 MeshFlags = MDLSPM_CHUNK_NONE;
    
    if (isGouraudShading_)
        MeshFlags |= MDLSPM_CHUNK_GOURAUDSHADING;
    
    File_->writeValue<s16>(MeshFlags);
...
void MeshSaverSPM::writeChunkSurface(const u32 Surface)
{
    ...
    // Set the flags
    s16 SurfaceFlags = MDLSPM_CHUNK_NONE;
    ...
    File_->writeValue<s16>(SurfaceFlags);
...
void MeshSaverSPM::writeChunkAnimationBone(const scene::AnimationBone* Bone)
{
