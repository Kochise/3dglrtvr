...
MeshLoader3DS::MeshLoader3DS()
    : MeshLoader(), CurChunk_(0), CurObjGroup_(0), CurJoint_(0)
{
    init();
}
MeshLoader3DS::~MeshLoader3DS()
...
video::color MeshLoader3DS::readColor()
{
    ...
    switch (TmpChunk.ID)
    {
        case CHUNK_COLOR_F32:
            Color.Red   = (u8)(readValue<f32>() * 255);
            Color.Green = (u8)(readValue<f32>() * 255);
            Color.Blue  = (u8)(readValue<f32>() * 255);
            break;
...
void MeshLoader3DS::buildMesh(const SObjectGroup3DS &ObjectGroup)
{
    ...
    else if (!ObjectGroup.TriangleList.empty())
    {
        ...
        // Create each triangle with its three vertices
        for (std::vector<scene::SMeshTriangle3D>::const_iterator it = ObjectGroup.TriangleList.begin();
    ...
    if (Joint)
    {
        ...
        scene::AnimationNode* Anim = (scene::AnimationNode*)ObjectGroup.Object->addAnimation(scene::ANIMATION_NODE);
        ...
        for (u32 i = 0; i < MaxCount; ++i)
        {
            ...
            Anim->addSequence(Position, Rotation, Scale);
        }
    }
}
