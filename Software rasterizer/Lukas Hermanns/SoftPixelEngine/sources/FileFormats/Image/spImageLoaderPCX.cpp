...
ImageLoaderPCX::ImageLoaderPCX(const io::stringc &Filename) : ImageLoader(Filename)
{
}
...
SImageData* ImageLoaderPCX::loadImageData()
{
    ...
    // Flip Y axis
    ImageConverter::flipImageVert(texture->ImageBuffer, texture->Width, texture->Height);
...
void ImageLoaderPCX::readPCX1bit(SHeaderPCX* pHeader, SImageData* texture)
{
    s32 y, i, bytes;
    s32 colorIndex;
...
void ImageLoaderPCX::readPCX4bit(SHeaderPCX* pHeader, SImageData* texture)
{
    u8* colorIndex, * line;
    u8* pLine, * ptr;
    s32 rle_count = 0, rle_value = 0;
...
void ImageLoaderPCX::readPCX8bit(SHeaderPCX* pHeader, SImageData* texture)
{
    s32 rle_count = 0, rle_value = 0;
    u8 palette[768];
    u8 magic;
...
void ImageLoaderPCX::readPCX24bit(SHeaderPCX* pHeader, SImageData* texture)
{
    u8* ptr = texture->ImageBuffer;
    s32 rle_count = 0, rle_value = 0;
    s32 y, c;
