...
void TextureManipulator::drawMosaic(video::Texture* Tex, s32 PixelSize, dim::rect2di Rect)
{
    ...
    for (dim::point2di Pos(0, Rect.Top); Pos.Y < Rect.Bottom; ++Pos.Y)
    {
        for (Pos.X = Rect.Left; Pos.X < Rect.Right; ++Pos.X)
        {
            Tex->setPixelColor(
                Pos,
                Tex->getPixelColor(dim::point2di( ( Pos.X / PixelSize ) * PixelSize, ( Pos.Y / PixelSize ) * PixelSize ))
            );
        }
    }
    ...
    for (dim::point2di Pos(0, Rect.Top); Pos.Y < Rect.Bottom; ++Pos.Y)
    {
        for (Pos.X = Rect.Left; Pos.X < Rect.Right; ++Pos.X)
        {
            ...
            for (c = 0; sy <= h; ++sy)
            {
                ...
                for (; sx <= w; ++sx, ++c)
                {
                    Color = NewTexture->getPixelColor(Pos + dim::point2di(sx, sy));
                    r += Color.Red;
                    g += Color.Green;
                    b += Color.Blue;
                }
            ...
            Tex->setPixelColor(Pos, video::color(r/c, g/c, b/c));
        }
    }
