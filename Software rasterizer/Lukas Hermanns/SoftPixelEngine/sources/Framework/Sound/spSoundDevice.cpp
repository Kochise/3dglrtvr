...
SoundDevice::SoundDevice(const ESoundDevices DeviceType) : DeviceType_(DeviceType)
{
...
{
    return "SensitiveSoundSystem - v.3.1";
}

io::stringc SoundDevice::getInterface() const
{
    return "";
}

Sound* SoundDevice::loadSound(const io::stringc &Filename, const ESoundModes Mode, u8 BufferCount)
{
    Sound* NewSound = createSound(Mode);
...
{
    for (std::list<Sound*>::iterator it = SoundList_.begin(); it != SoundList_.end(); ++it)
    {
        if ((*it)->looped() && (*it)->finish())
            (*it)->play(true);
        if ((*it)->getVolumetric())
            updateSoundVolumetic(*it);
    }
}
...
void SoundDevice::playMelody(const ENoteNames Note, u8 Octave, u32 Duration, u32 Delay)
{
    ...
    const s32 Frequence = (s32)( pow(FreqFactor, Note + Octave * (NOTE_H + 1)) * FreqBase );
    
    Duration    = (s32)(MelodySpeed_ * Duration);
    Delay       = (s32)(MelodySpeed_ * Delay);
...
void SoundDevice::playMelody(io::stringc MelodyCmdStr)
{
    ...
    while ( ( pos1 = MelodyCmdStr.find(";", pos2) ) != -1 )
    {
        ...
        if (Note != -1)
        {
            playMelody((ENoteNames)Note, Octave, Duration, Delay);
            Note = -1;
        }
