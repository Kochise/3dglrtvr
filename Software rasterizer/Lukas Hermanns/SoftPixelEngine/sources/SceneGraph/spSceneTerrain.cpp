...
void Terrain::init()
{
    ...
    RootTreeNode_   = new TreeNodeQuad();
...
void Terrain::createQuadTree(TreeNodeQuad* Node, s32 &MIPLevel, dim::point2di CurPos)
{
...
void Terrain::createTreeNodeMesh(
    TreeNodeQuad* Node, STreeNodeData* NodeData, const dim::point2di &CurPos)
{
    /* Temporary variables */
    s32 x, y, i, j, k;
...
void Terrain::selectTreeNodeMesh(TreeNodeQuad* Node)
{
    /* Temporary variables */
    ...
    const f32 d = math::getDistance(
        GlobalCamPosition_,
        dim::vector3df(NodeData->Center.X, 0.0f, NodeData->Center.Y) * Scale_.getScale() + Position_.getPosition()
    );
    
    /* Add to the render node list */
    if (d > 7.0f * (GeoMIPLevels_ - NodeData->MIPLevel) || Node->isLeafNode() ||
...
void Terrain::renderTreeNodeMesh(TreeNodeQuad* Node)
{
    /* Temporary variables */
    STreeNodeData* NodeData = (STreeNodeData*)Node->getUserData();
...
bool Terrain::deformTreeNodeMesh(TreeNodeQuad* Node, const dim::point2df &Pos)
{
    return getNodeLevel(RootTreeNode_, Pos) < static_cast<STreeNodeData*>(Node->getUserData())->MIPLevel;
}

s32 Terrain::getNodeLevel(const TreeNodeQuad* Node, const dim::point2df &Pos)
{
    /* Temporary variables */
    STreeNodeData* NodeData = (STreeNodeData*)Node->getUserData();
    
    /* Check if the node is selected */
    if (!Node->isLeafNode() && !NodeData->Selected)
    {
        s32 i;
