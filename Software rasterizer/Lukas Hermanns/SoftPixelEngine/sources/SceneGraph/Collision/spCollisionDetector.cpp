...
{
    Matrix  = Object->getGlobalLocation();
    ...
    SceneNode* Object, Collision* CollisionObject, f32 Radius, const dim::aabbox3df &BoundingBox)
{
    ...
    for (std::list<SCollisionObject*>::iterator it = CollObjectList_.begin(); it != CollObjectList_.end(); ++it)
    {
        if ((*it)->CollisionHandle == CollisionObject && (*it)->Object == Object)
        {
            /* Update the configuration */
            (*it)->Radius       = Radius;
            (*it)->BoundingBox  = BoundingBox;
            (*it)->deleteTriangleData();
            return *it;
        }
    }
...
SCollisionObject* CollisionDetector::addCollisionMesh(Mesh* Mesh, Collision* CollisionObject, const video::EFaceTypes FaceType)
{
    ...
    for (std::list<SCollisionObject*>::iterator it = CollObjectList_.begin(); it != CollObjectList_.end(); ++it)
    {
        if ((*it)->CollisionHandle == CollisionObject && (*it)->Object == Mesh)
        {
            /* Recreate the triangle data */
            (*it)->createTriangleData();
            (*it)->FaceType         = FaceType;
            (*it)->CollisionHandle  = CollisionObject;
            return *it;
        }
    }
    ...
    SceneNode* Object, const EPickingTypes Type, f32 Radius, const dim::aabbox3df &BoundingBox)
{
    ...
    for (std::list<SPickingObject*>::iterator it = PickObjectList_.begin(); it != PickObjectList_.end(); ++it)
    {
        if ((*it)->Object == Object)
        {
            /* Update the configuration */
...
bool CollisionDetector::pickIntersection(const dim::vector3df &PosA, const dim::vector3df &PosB)
{
    ...
    for (std::list<SPickingObject*>::iterator it = PickObjectList_.begin(); it != PickObjectList_.end(); ++it)
    {
        ...
        if (!(*it)->Object->getVisible() && !(*it)->IgnoreVisibility)
            continue;
        ...
        switch ((*it)->Type)
        {
            ...
            {
                if (checkInviewLineSphere(*it, Line))
                    return false;
            }
            break;
            ...
            {
                if ((*it)->Mesh->getOctTreeRoot())
                {
                    if (checkInviewLinePolygonTree(*it, Line))
                        return false;
                }
                else
                {
                    if (checkInviewLinePolygon(*it, Line))
                        return false;
                }
            }
            break;
...
bool CollisionDetector::meshIntersection(Mesh* MeshA, Mesh* MeshB)
{
    ...
    /*TreeNodeOct* TreeA = MeshA->getOctTreeRoot();
    TreeNodeOct* TreeB = MeshB->getOctTreeRoot();
...
SCollisionContact CollisionDetector::getPolygonCollisionContact(
...
void CollisionDetector::processContact(
    SCollisionObject* CurObject, const SCollisionContact &Contact, const dim::vector3df &Position)
{
...
void CollisionDetector::processPicking(const dim::line3df &Line)
{
    for (std::list<SPickingObject*>::iterator it = PickObjectList_.begin(); it != PickObjectList_.end(); ++it)
    {
        ...
        {
            case PICKMODE_SPHERE:
                checkIntersectionLineSphere(*it, Line); break;
            
            case PICKMODE_BOX:
                checkIntersectionLineBox(*it, Line); break;
            
            case PICKMODE_POLYGON:
            {
                if ((*it)->Mesh->getOctTreeRoot())
                    checkIntersectionLinePolygonTree(*it, Line);
                else
                    checkIntersectionLinePolygon(*it, Line);
            }
            break;
        }
    } // next picking object
...
void CollisionDetector::checkCollisionSphereToPolygon(SCollisionObject* CurObject, SCollisionObject* DestObject)
{
    ...
    if (math::getDistanceSq(Line.Start, Line.End) > RadiusSq)
    {
        ...
        for (s32 i = 0; i < DestObject->TriangleCount; ++i)
        {
            ...
            {
                SCollisionContact Contact = getPolygonCollisionContact(CurObject, DestObject, i, Triangle, CollisionPoint);
...
void CollisionDetector::checkCollisionSphereToPolygonTree(SCollisionObject* CurObject, SCollisionObject* DestObject)
{
    ...
    std::list<const TreeNodeOct*> TreeNodeList;
    ...
    if (math::getDistanceSq(Line.Start, Line.End) > RadiusSq)
    {
        ...
        for (std::list<const TreeNodeOct*>::iterator itTree = TreeNodeList.begin(); itTree != TreeNodeList.end(); ++itTree)
        {
            ...
            for (it = TriangleList->begin(); it != TriangleList->end(); ++it)
            {
                ...
                if (math::CollisionLibrary::checkLineTriangleIntersection(Triangle, Line, CollisionPoint))
                {
                    SCollisionContact Contact = getPolygonCollisionContact(CurObject, DestObject, it, Triangle, CollisionPoint);
...
void CollisionDetector::checkIntersectionLineBox(SPickingObject* CurObject, const dim::line3df &Line)
{
    ...
    dim::matrix4f Mat(CurObject->Object->getGlobalLocationInverse());
...
void CollisionDetector::checkIntersectionLinePolygon(SPickingObject* CurObject, const dim::line3df &Line)
{
    ...
    CollMatrix_ = CurObject->Object->getGlobalLocation();
...
bool CollisionDetector::checkIntersectionMeshMesh(Mesh* MeshA, Mesh* MeshB)
{
    ...
    dim::matrix4f MatA(MeshA->getGlobalLocation()), MatB(MeshB->getGlobalLocation());
    dim::matrix4f TmpMat;
...
bool CollisionDetector::checkIntersectionMeshMesh(Mesh* MeshA, Mesh* MeshB, TreeNodeOct* TreeNodeA)
{
...
bool CollisionDetector::checkIntersectionMeshMesh(Mesh* MeshA, Mesh* MeshB, TreeNodeOct* TreeNodeA, TreeNodeOct* TreeNodeB)
{
