...
{
    RECT rc;
    GetWindowRect(*(HWND*)__spDevice->getWindowHandle(), &rc);
    
    SetCursorPos(
        __Screen_X + Position.X + rc.left,
        __Screen_Y + Position.Y + rc.top
    );
...
dim::point2di InputControl::getCursorPosition() const
{
    ...
    return dim::point2di(Position.x - rc.left - __Screen_X, Position.y - rc.top - __Screen_Y);
}
