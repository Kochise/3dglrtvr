...
BasicMeshGenerator::BasicMeshGenerator() : Surface_(0)
{
}
    ...
    Mesh* Mesh, const u32 Surface, const dim::vector3df AnchorPoints[4][4], const s32 Segments, bool isFrontFace)
{
    ...
    dim::vector3df* VecList = new dim::vector3df[Segments + 1];
    ...
    {
        py  = (f32)u / (f32)Segments;
        lpy = ((f32)u - 1.0f) / (f32)Segments;
...
Mesh* BasicMeshGenerator::createHeightField(const video::Texture* TexHeightMap, const s32 Segments)
{
    ...
    for (z = 0; z <= Segments; ++z)
    {
        ...
        {
            y = TexHeightMap->getPixelColor(
                dim::point2di(s32(size*x*Width), s32(size*z*Height))
            ).getBrightness<f32>() / 255;
            
            addVertex(
                (f32)x*size - 0.5f, y, -(f32)z*size + 0.5f, (f32)x*size, (f32)z*size
            );
        }
    }
...
void BasicMeshGenerator::createIcosphere()
{
    ...
    for (std::list<dim::triangle3df>::iterator it = Triangles.begin(); it != Triangles.end(); ++it)
        addFace(it->PointA, it->PointB, it->PointC);
...
void BasicMeshGenerator::createTorusknot()
{
    ...
    */
    ...
    dim::vector3df* RingCenters = new dim::vector3df[Stacks];
...
#   else
extern f32 __spTeapotVertices[];
extern s32 __spTeapotIndices[];
#   endif
...
{
    return pow( pow( math::Abs<f32>( (f32)COS(m*phi/4)/a ) , n2 ) + pow( math::Abs<f32>( (f32)SIN(m*phi/4)/b ) , n3 ) , (1/(-n1)) );
}
    ...
    bool FaceLinkCCW)
{
    ...
    {
        v = dirv * (f32)y / DetailVert;
        
        for (x = 0; x <= DetailHorz; ++x, ++c)
        {
            u = diru * (f32)x / DetailHorz;
            ...
            addVertex(
                ...
                // Texture coordinate
                t0.X + (t1.X - t0.X) * (f32)x/DetailHorz,
                t0.Y + (t1.Y - t0.Y) * (f32)y/DetailVert
            );
        }
    }
